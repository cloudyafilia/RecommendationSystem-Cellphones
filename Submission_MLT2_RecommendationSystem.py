...
"""Data duplikat dihapus berdasarkan kolom `cellphone_id` untuk memastikan setiap item unik sebelum pemodelan Content-Based Filtering. Hal ini mencegah bias dan inkonsistensi data.
...
"""Dibuat dataframe `phone_new` yang berisi fitur utama ponsel (`cellphone_id`, `brand`, `model`, `operating_system`) sebagai input untuk model Content-Based Filtering. Langkah ini memudahkan transformasi dan perhitungan kemiripan antar produk.
...
"""Transformasi fitur teks seperti `brand` menggunakan TF-IDF Vectorizer untuk mengubah data teks menjadi representasi numerik berbobot. Representasi ini digunakan untuk menghitung kemiripan antar produk dalam Content-Based Filtering.
...
### Encoding pada fitur user_id dan cellphone_id
"""
...
"""Kolom `user_id` dan `cellphone_id` di-encoding ke format numerik untuk keperluan model Collaborative Filtering berbasis embedding. Proses ini diperlukan agar model dapat mengolah data pengguna dan produk secara efisien.

### Melakukan mapping pada fitur user_id dan cellphone_id
"""
...
"""### Menampilkan jumlah user dan cellphone serta sebaran nilai rating"""
...
"""Pada tahap ini, kolom `rating` diubah menjadi tipe data `float32` untuk memastikan bahwa data dapat diproses dengan tepat dalam model machine learning. Konversi tipe ini tidak mengubah rentang nilai rating, hanya mengoptimalkan penyimpanan dan kompatibilitas data. Normalisasi tidak dilakukan karena model dapat menangani nilai asli rating dengan baik dan agar hasil prediksi tetap mudah diinterpretasi dalam skala asli. Selain itu, nilai minimum dan maksimum rating juga dihitung untuk memberikan gambaran tentang sebaran nilai rating dalam dataset. Dari hasil, terlihat bahwa nilai rating berkisar dari 1.0 hingga 18.0, di mana nilai 18.0 merupakan outlier karena melebihi skala rating yang umum (biasanya 1-10), sehingga perlu ditangani lebih lanjut.
...
"""Menghapus outlier pada kolom rating, seperti nilai di luar rentang valid, yaitu 18 agar data interaksi yang digunakan bersih dan valid.
...
"""Dataset diacak menggunakan fungsi sample(frac=1, random_state=42) untuk memastikan distribusi data yang merata dan menghindari bias saat proses pembagian data menjadi training dan testing.
...
"""Melakukan mapping data user dan cellphone menjadi satu value agar dapat diproses oleh model embedding dalam Collaborative Filtering dengan efisien.
...
"""Melakukan pembagian data menjadi data train sebanyak 791 baris (80%) dan data test sebanyak 198 baris (20%).
...
## 1. Menghitung derajat kesamaan (similarity degree) antar model dengan teknik cosine similarity
"""
...
"""
## 2. Membuat matriks kesamaan setiap model dengan menampilkan nama model dalam 33 sampel kolom (axis = 1) dan 10 sampel baris (axis=0)"""
...
"""## 3. Membuat fungsi model_recommendations"""
...
"""## 4. Menampilkan hasil rekomendasi untuk beberapa sampel"""
...
"""## 2. Melakukan inisialisasi model dan compile"""
...
"""## 3. Melakukan training terhadap model"""
...
"""## 4. Visualisasi metrik dan evaluasi model"""
...
## 5. Membuat variabel cellhpone_not_reviewed sebagai daftar cellphone untuk direkomendasikan pada pengguna
"""
...
"""## 6. Memperoleh hasil rekomendasi cellphone"""
